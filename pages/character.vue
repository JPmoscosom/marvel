    ...
    <v-container v-for="char in character" :key="char.id" fluid fill-height>
      <v-row>
        <v-card class="d-flex elevation-5 my-6 mx-5" width="100%">
          <v-col cols="12" md="4">
            <v-img class="rounded" :src="char.thumbnail.path + '.' + char.thumbnail.extension"
                width="400"
                height="400"
            >
            </v-img>
      ...
      <v-row align="center" justify="center">
        <v-card class="d-flex flex-wrap elevation-5 my-6 mx-5" max-width="95%">
            <v-slide-group center-active show-arrows class="pa-4">
                ...
                <v-slide-group-item v-for="comic in comics" :key="comic.id" class="ml-3">
                    <v-card max-width="350" min-height="530" class="d-flex flex-column">
                        <v-img :src="comic.thumbnail.path + '.' + comic.thumbnail.extension" max-height="530">
...
export default {
  ...
  created() {
    ...
    const API = '?ts=7&apikey=7e09e88ea35fdcfd907fc60e66efb209&hash=2556b952565b6bd248a714ffc4786cee'
    let url = `${URL}` +`${id}` + `${API}`
